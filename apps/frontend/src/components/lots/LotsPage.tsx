...
const LotsPage: React.FC = () => {
  ...
  const lotStatus = (account?: AccountItem | null) => {
    ...
    if (hasFlag(account.rental_frozen)) return { label: "Заморожено", className: "bg-slate-100 text-slate-700" };
    if ((account.owner || "").trim()) return { label: "В аренде", className: "bg-amber-50 text-amber-700" };
    if (hasFlag(account.account_frozen)) return { label: "Заморожен", className: "bg-rose-50 text-rose-700" };
    if (hasFlag(account.low_priority)) return { label: "Низкий приоритет", className: "bg-rose-50 text-rose-600" };
