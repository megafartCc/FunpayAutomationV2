...
def _ensure_funpayapi_on_path() -> None:
    candidates: list[Path] = []
    env_repo = os.getenv("FUNPAY_REPO_ROOT")
    if env_repo:
        candidates.append(Path(env_repo).expanduser().resolve())
    env_pkg = os.getenv("FUNPAY_API_PATH")
    if env_pkg:
        candidates.append(Path(env_pkg).expanduser().resolve())

    root = _resolve_repo_root()
    candidates.append(root)
    candidates.append(root / "workers" / "funpay")

    current = Path(__file__).resolve()
    for parent in current.parents:
        candidates.append(parent)
        candidates.append(parent / "workers" / "funpay")

    for base in candidates:
        if not base:
            continue
        funpay_dir = base / "FunPayAPI"
        if funpay_dir.is_dir():
            sys.path.insert(0, str(base))
            return

    raise ModuleNotFoundError(
        "FunPayAPI package not found. Set FUNPAY_API_PATH or FUNPAY_REPO_ROOT to the repo root."
    )


ROOT = _resolve_repo_root()
sys.path.insert(0, str(ROOT))
_ensure_funpayapi_on_path()
